...
// Mirrors the PRODUCTION CSP headers (firebase.json). The app is fully
// offline-first since issue #11: script-src/connect-src are 'self' only —
// zero external origins — so the whole experience works offline.
const CSP_HEADER = "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; font-src 'self' data:; img-src 'self' data:; connect-src 'self'; frame-src 'self'; object-src 'none'; base-uri 'self'; worker-src 'self';";
...
async function run() {
  ...
  // 4. Offline isolation: the app must make ZERO external network calls —
  //    no gstatic/googleapis hosts in the precache (the Firebase SDK was
  //    removed in issue #11; the app is 100% same-origin offline-first).
  check('Offline: precache contains no external (gstatic/googleapis) entries', await page.evaluate(async () => {
    const keys = await caches.keys();
    const cache = await caches.open(keys.find((k) => k.startsWith('marrow-planner-pwa')) || keys[0]);
    const reqs = await cache.keys();
    return !reqs.some((r) => /gstatic\.com|googleapis\.com|google\.com/.test(r.url));
  }), 'no cross-origin entries in precache');
  check('Offline: app loaded no external scripts (page performance entries)', await page.evaluate(() =>
    performance.getEntriesByType('resource')
      .filter((e) => !e.name.startsWith(location.origin))
      .length === 0
  ), 'all resource entries same-origin');
