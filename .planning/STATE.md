...
- **Status**: v218 deployed and live at flowmd-04.web.app — a fresh device now opens the edition that has data instead of the cloud's last-writer `activeSource` when that points at an empty partition (data-aware redirect in `mergeCloudPerField`, including the equal-default case); the two-device sync harness defaults to the production `'Dr. Aspirant'` profile so device defaults can no longer race each other, with tests locking in fresh-device name adoption, the empty-cloud-profile guard, and a zero-write three-device settle. Unit 65/65, full suite green.
...
- **Git note**: `main` at v218 — pushed to origin, deployed live.
...
## Notes
- Cache-busting version: v218 (index.html `?v=218`); live site serving v218
